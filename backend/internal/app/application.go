...
func New(ctx context.Context, cfg config.Config, logger *slog.Logger) (*Application, error) {
	...
	selector.SetUsageObserver(auditRepo)
	...
	selector.UpdateActiveSetSelector(cfg.Routing.ActiveSetSelectorEnabled, cfg.Routing.ActiveSetSelectorSize)
	...
	settingsService := settingsapp.NewService(cfg, settingsUpdatedAt, settingsRevision, runtimeSettingsRepo, notifySettings, func(next config.Config) {
		...
		selector.UpdateActiveSetSelector(next.Routing.ActiveSetSelectorEnabled, next.Routing.ActiveSetSelectorSize)
